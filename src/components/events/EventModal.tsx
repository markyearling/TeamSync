...
const EventModal: React.FC<EventModalProps> = ({ event, onClose, mapsLoaded, mapsLoadError, userTimezone = 'UTC', onEventUpdated, shouldOpenMessages = false }) => {
  ...
  const modalContainerClasses = isNative
    ? "fixed inset-0 z-[200] bg-white dark:bg-gray-800"
    : "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[200]";

  const modalContentClasses = isNative
    ? "flex flex-col h-full w-full overflow-hidden bg-white dark:bg-gray-800 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] transform translate-z-0"
