/*
  # Update message notification trigger to respect conversation read tracking

  1. Changes
    - Update `create_message_notification` to check if recipient is actively viewing
    - Use conversation_participants table to track if recipient has modal open
    - Only send notifications if recipient hasn't read recently (not actively viewing)
    - This prevents spam when both users have the modal open
    
  2. Logic
    - Check recipient's last_read_at timestamp
    - If last_read_at is within last 30 seconds, assume they're actively viewing
    - Don't send notification if actively viewing
    - Send notification if not actively viewing or no recent read activity
*/
...
BEGIN
  ...
  SELECT 
    CASE 
      WHEN participant_1_id = v_recipient_id THEN participant_1_last_read_at
      ELSE participant_2_last_read_at
    END INTO v_recipient_last_read
  FROM conversation_participants
  WHERE conversation_id = NEW.conversation_id;

  -- If they read within last 30 seconds, they're actively viewing
